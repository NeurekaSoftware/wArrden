...
public class RadarrV3Client : IArrClient
{
    ...
    public RadarrV3Client(HttpClient http, string instanceName)
    {
    ...
    public async Task<IReadOnlyList<QueueResource>> GetQueueAsync(CancellationToken ct)
    {
        ...
        while (true)
        {
            var url = $"api/v3/queue?includeUnknownMovieItems=true&page={page}&pageSize={pageSize}";
    ...
    {
        using var response = await _http.DeleteAsync($"api/v3/queue/{queueId}?blocklist=true&skipRedownload=false", ct);
        response.EnsureSuccessStatusCode();
    }

    public async Task DeleteQueueItemWithoutBlocklistAsync(int queueId, CancellationToken ct)
    {
        using var response = await _http.DeleteAsync($"api/v3/queue/{queueId}?blocklist=false&skipRedownload=false", ct);
        response.EnsureSuccessStatusCode();
    }
    ...
    {
        var byId = new Dictionary<int, WantedMovieResource>(capacity: 100);
        var page = 1;
        const int pageSize = 100;

        while (true)
        {
            var url = $"api/v3/wanted/{type}?monitored=true&page={page}&pageSize={pageSize}&sortKey=lastSearchTime&sortDirection=ascending";
            using var response = await _http.GetAsync(url, ct);
            response.EnsureSuccessStatusCode();
    ...
    public Task<bool> EnsureTagOnSeriesAsync(int seriesId, int tagId, CancellationToken ct)
        => throw new NotSupportedException("Radarr does not support series tags.");
    ...
    public Task<bool> EnsureTagOnArtistAsync(int artistId, int tagId, CancellationToken ct)
        => throw new NotSupportedException("Radarr does not support artist tags.");

    public Task<HashSet<int>> ResolveSeriesIdsAsync(int[] episodeIds, CancellationToken ct)
        => throw new NotSupportedException("Radarr does not support series resolution.");

    public Task<HashSet<int>> ResolveArtistIdsAsync(int[] albumIds, CancellationToken ct)
        => throw new NotSupportedException("Radarr does not support artist resolution.");
