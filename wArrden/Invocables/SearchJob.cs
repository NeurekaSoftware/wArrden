...
public class SearchJob : IInvocable
{
    ...
    public SearchJob(SearchService search, IArrClient client,
        string searchKind, string instanceType, int maxResults, string cooldownStr, bool isDryRun)
    {
        _search = search;
        _client = client;
    ...
    public Task Invoke()
    {
        ...
        {
            ("missing", "sonarr") => _search.SearchMissingEpisodesAsync(_client, _maxResults, _cooldown, _isDryRun, ct),
            ("upgrade", "sonarr") => _search.SearchUpgradeEpisodesAsync(_client, _maxResults, _cooldown, _isDryRun, ct),
